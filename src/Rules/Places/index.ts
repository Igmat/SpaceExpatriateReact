...
export abstract class BasicPlace<T extends BasicCard = BasicCard> {
  ...
  constructor() {
    ...
    // makeAutoSavable(["_cards" as any]);
  }

  protected abstract getCardInstance(id: number, type: CardType): T;

  get cards(): readonly T[] {
    return this._cards.map((card) => this.getCardInstance(card.id, card.type));
  }
  // выписываем карту
  takeCard(id: number, type: CardType) {
    this._cards = this._cards.filter((el) => el.id !== id || el.type !== type);
  }
  // вписываем карту
